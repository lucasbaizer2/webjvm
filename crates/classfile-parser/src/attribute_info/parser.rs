...
fn method_parameter_parser(input: &[u8]) -> Result<(&[u8], MethodParameter), Err<&[u8]>> {
    do_parse!(
        input,
        name_index: be_u16
            >> access_flags: be_u16
            >> (MethodParameter {
                name_index,
                access_flags: crate::field_info::FieldAccessFlags::from_bits_truncate(access_flags),
            })
    )
}
...
pub fn method_parameters_attribute_parser(
    ...
    do_parse!(
        ...
        parameters_count: be_u8
            >> parameters: count!(method_parameter_parser, parameters_count as usize)
